...
GithubClient.prototype.branchesRequestEndpoint = function () {
  return this.endpoint + '/repos/' + this.owner + '/' + this.repo + '/branches'
}
...
GithubClient.prototype.prepareReleaseBranch = function (version) {
  const self = this
  version = version || 1
  const branchName = self.releaseBranch + '/' + version

  return self
    .get(self.branchesRequestEndpoint() + '/' + branchName)
    .then(function (res) {
      if (res.statusCode !== 404) {
        return Promise.reject(new Error('Branch already exists: ' + branchName))
      }

      return self.get(self.getRefEndpoint(self.head))
    })
    .then(function (res) {
